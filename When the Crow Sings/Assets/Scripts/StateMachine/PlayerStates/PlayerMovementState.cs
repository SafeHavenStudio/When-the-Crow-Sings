...
public class PlayerMovementState : StateMachineState
{
    ...
    public override void Update(float deltaTime)
    {
        ...
        if (s.isSprintingButtonHeld && inputMagnitude > 0f || GameSettings.GetModel().autoRun && inputMagnitude > 0f && !s.isCrouchingToggled)
        {
    ...
    {
        if (ServiceLocator.Get<GameStateManager>().currentLevelDataLVL.canThrowBirdseed)
